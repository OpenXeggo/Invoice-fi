...
const CreateInvoice = ({ contract, account }) => {
  ...
  useEffect(() => {
    if (isSupported && chainId) {
      setToken(networks[chainId].coins[0]);
    }
  }, [chainId, isSupported]);

  const [token, setToken] = useState({ name: "", address: "" });
  ...
  const onSubmitHandler = async (e) => {
    try {
      ...
      console.log(convertToWei, "convertoWei");
      await contract.methods
        .createInvoice("0xCDDF9929Cd215D5338549D44dA81b1f41a272b48", convertToWei, customerAddress)
        .send({
          from: account,
          gasPrice: initWeb3().utils.toWei("40", "gwei"),
        });
      const id = await contract.methods
        .createInvoice(token.address, convertToWei, customerAddress)
        .call();
      setInvoiceId(id - 1);
      ...
      <div className="page-content">
        <div className="invoice-details flex">
          ...
          <div className="right-details flex flex-col gap-10">
            ...
            <DateDetail value={dueDate} setValue={setDueDate} />
          </div>
        </div>
        <div className="token-details">
          <table className="items-table">
            <thead>
              <tr>
                <th style={{ textAlign: "start", minWidth: "300px" }}>
                  Item Description
                </th>
                <th>Quantity</th>
                <th>Unit Price</th>
              ...
              <tbody key={i}>
                <tr>
                  ...
                  <td>
                    <input
                      value={row.quantity}
                      onChange={(e) => handleRow(e, i, "quantity")}
                      type="number"
                      placeholder="-"
                  ...
                  <td>
                    <input
                      value={row.price}
                      onChange={(e) => handleRow(e, i, "price")}
                      type="number"
                      placeholder="-"
                      className="center-input"
                    />
                  ...
                  <td className="relative">
                    <input
                      type="text"
                      value={row.vat_rate}
                      onChange={(e) => handleRow(e, i, "vat_rate")}
                      placeholder="-"
                      className="center-input pr-10"
                    />
                  ...
                  <td>
                    <input
                      type="number"
                      value={amounts[i].vat_amount}
                      onChange={(e) => handleRow(e, i, "vat_amount")}
                      placeholder="-"
                      className="center-input"
                    />
                  </td>
                  <td>
                    <input
                      type="number"
                      value={amounts[i].gross_amount}
                      onChange={(e) => handleRow(e, i, "gross_amount")}
                      placeholder="-"
                      className="center-input"
                    />
                  </td>
                  {i > 0 && (
                    <td>
                      <span className="pointer" onClick={() => removeRow(i)}>
          ...
          <div className="add-item" onClick={() => addRows()}>
            {" "}
            <span>
        ...
        <div className="payment-container">
              ...
              <div className="flex gap-10">
                {Celo.map((celotoken, i) => (
                  <div
                    key={i}
                    className="select"
                    onClick={() => setToken({ ...celotoken })}
                  >
                    <span>{celotoken.name}</span>
                    {celotoken.name === token.name && (
                      <span>
                        <img src={TickIcon} />
                      </span>
                    )}
          ...
          <div className="invoice-buttons">
            <div className="flex gap-20">
              ...
              <button className="xeggo-btn" onClick={onSubmitHandler}>
        ...
        {link.length > 1 && (
          <LinksModal link={link} closeModal={closeLinkModal} />
        )}
      </div>
    </div>
  );
