 ...
 */
contract XeggoInvoice is Pausable,Owner {
    ...
    mapping(address=>uint) public proceeds; // Mapping of user to their cooresponding native balances paid by inoice receiver
     ...
     * @return nativeCurrency  to be paid 
    ...
    function getInvoice(
        uint invoiceID
            ...
            return (invoices[invoiceID]._invoiceCreator, invoices[invoiceID]._tokenAddress, invoices[invoiceID]._tokenAmountInWei,invoices[invoiceID]._nativeCurrencyAmount, invoices[invoiceID]._receiver, invoices[invoiceID]._isPaidBy, invoices[invoiceID]._isCancelled );
    }
     ...
     *
     * @param tokenAddress Address of the token which token is to be transferred, 0x00 if the invoice is to be paid in native currency
     * @param tokenAmountInWei Number of tokens to be paid in smallest decimal of the token, 0 if the invoice is to be paid in native currency
     * @param nativeCurrency for the amount to be paid in native L1 currency eg eth, matic, 0 if the invoice is to be paid in tokens
     *
    ...
    function createInvoice(
        ...
        uint nativeCurrency
            ...
            return createInvoice(tokenAddress,tokenAmountInWei,nativeCurrency,_msgSender());
    }
    ...
    function createInvoice(
        ...
        address receiver
            ...
            require(tokenAmountInWei!=0 || nativeCurrency!=0,"Either one TokenAmount or Native Curreny must be greater than 0");
            require(invoices[_invoiceID]._invoiceCreator == address(0), "Invoice ID already used");
            if(nativeCurrency==0){
                require(tokenAmountInWei>0 && tokenAddress!=address(0),"Token amount must be greater than 0");
            }
            if(nativeCurrency>0){
                                require(tokenAmountInWei==0 && tokenAddress==address(0),"Token amount and address must be 0");
            ...
            invoices[currentInvoiceID]._nativeCurrencyAmount      = nativeCurrency;
            ...
            emit InvoiceCreated(
                ...
                invoices[currentInvoiceID]._nativeCurrencyAmount,
    ...
    function payInvoice(
            ...
            invoice memory _invoice = invoices[invoiceID];
            if(_invoice._tokenAddress!=address(0) && _invoice._tokenAmountInWei!=0){
             require(canIPayTheInvoice(invoiceID), "You cannot pay for the invoice"); 
             require(msg.value==0,"Cannot pay with native currency for the invoice to be paid by ERC20 Token");     
            ...
            invoices[invoiceID]._isPaidBy = _msgSender();
               ...
               require(_invoice._nativeCurrencyAmount==msg.value,"The funds sent should be equal to the invoice pay amount");
               invoices[invoiceID]._isPaidBy = _msgSender();
               proceeds[invoices[invoiceID]._invoiceCreator]+=msg.value;
            }
    ...
    function withdrawProceeds() external {
        require(proceeds[msg.sender]>0,"Not enough funds available");
        payable(msg.sender).transfer(proceeds[msg.sender]);
